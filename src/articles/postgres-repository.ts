...
export class PostgresRepository implements Repository<Article> {
  ...
  async getAll(): Promise<Article[]> {
    const { rows } = await this.dbService.client.queryObject<
      Article
    >`SELECT _id, title, content, price FROM articles`;
    return rows;
  }

  async getById(id: string) {
    const { rows } = await this.dbService.client.queryObject<Article>(
      `SELECT _id, title, content, price FROM articles WHERE _id = '${id}'`,
    );
    return rows[0];
  }
  ...
  async create(article: Omit<Article, '_id'>) {
    const { rows } = await this.dbService.client.queryObject<Article>(
      `INSERT INTO articles (title, content,price) VALUES ('${article.title}', '${article.content}','${article.price}') RETURNING _id, title, content, price;`,
    );
    return rows[0];
  }
  ...
  async updateOne(articleId: string, article: Article) {
    const { rows } = await this.dbService.client.queryObject<Article>(
      `UPDATE articles SET title = '${article.title}', content = '${article.content}' WHERE _id = '${articleId}' RETURNING _id, title, content;`,
    );
    return rows[0];
  }
  ...
  async deleteAll() {
    return this.dbService.client.queryObject<Article>(`DELETE FROM articles`);
  }
}
